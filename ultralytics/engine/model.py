...
class Model(nn.Module, PyTorchModelHubMixin, repo_url="https://github.com/ultralytics/ultralytics", pipeline_tag="object-detection", license="agpl-3.0"):
    ...
    def train(
        ...
        **kwargs: Any,
        ...
        model_yaml = getattr(self.model, "yaml", {}) or {}
        geocr_config = model_yaml.get("geocr", {})
        if isinstance(geocr_config, dict) and bool(geocr_config.get("enabled", False)):
            args.setdefault("geocr_enabled", True)
            args.setdefault("gdr", geocr_config.get("gdr", {"enabled": False}))
            args.setdefault(
                "gdr_curriculum_cfg",
                geocr_config.get("gdr_curriculum_cfg", "ultralytics/cfg/geocr/gdr_curriculum.yaml"),
            )
        if args.get("resume"):
