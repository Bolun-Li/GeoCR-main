...
class BaseMixTransform:
    ...
    def __call__(self, labels):
        ...
        prompt_im_files = []
        for label in [labels] + mix_labels:
            prompt_im_files.extend(label.get("prompt_im_files", []) or ([label.get("im_file")] if label.get("im_file") else []))
        ...
        if prompt_im_files:
            labels["prompt_im_files"] = prompt_im_files
...
class MixUp(BaseMixTransform):
    ...
    def __init__(self, dataset, pre_transform=None, p=0.0) -> None:
        ...
        Examples:
            ...
            >>> dataset = YOLODataset("path/to/URPC2019.yaml")
...
def v8_transforms(dataset, imgsz, hyp, stretch=False):
    ...
    if dataset.use_keypoints:
        ...
        if len(flip_idx) == 0 and hyp.fliplr > 0.0:
            hyp.fliplr = 0.0
            LOGGER.warning("WARNING ⚠️ No 'flip_idx' array defined in URPC2019.yaml, setting augmentation 'fliplr=0.0'")
        elif flip_idx and (len(flip_idx) != kpt_shape[0]):
            raise ValueError(f"URPC2019.yaml flip_idx={flip_idx} length must be equal to kpt_shape[0]={kpt_shape[0]}")
